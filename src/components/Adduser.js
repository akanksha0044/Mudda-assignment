...
function Adduser() {
  ...
  const pushdata = async (e) => {
    ...
    const { fname, lname, phone, email, password } = user;//destructuring
    const res = await fetch("/adduser", {
      ...
      body: JSON.stringify({
        ...
        fname, lname, phone, email, password, course, country, date//if both key value same, then write only one
      })
    });
    ...
    if (res.status === 201) {
      ...
      window.alert("Email already exist. If you want to update the details, you have to login first with this email id.");
    }
